...
function Post({data}){
  const{id,title,category,img,published,description,author}=data;
    ...
    return(
       ...
       <Link href={`/posts/${id}`}>  <Image src={img || ""}  width={300} height={250}  className="rounded" alt="section4"/></Link>
       </div>
       <div className="info flex justify-center flex-col">
       <div className="cat">
              <Link href={`/posts/${id}`} className="text-orange-600 hover:text-orange-800">{category ||"No Category"}</Link>
              <Link href={`/posts/${id}`} className="text-gray-800 hover:text-gray-600">- {published ||"Unlnown"}</Link>
          </div>
          <div className='title'>
